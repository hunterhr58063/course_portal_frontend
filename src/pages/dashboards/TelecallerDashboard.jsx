...
const AdminDashboard = () => {
    ...
    const isBaseDashboard = location.pathname === '/telecaller';
    ...
    useEffect(() => {
        const fetchStats = async () => {
            try {
                const res = await api.get('/dashboard/telecaller');
                setStats(res.data);
            } catch (err) {
        ...
        if (location.pathname === '/telecaller') {
            fetchStats();
        }
    }, [location.pathname]);

    const cards = [
        { title: 'Students', count: stats?.studentsCount ?? '-', link: '/telecaller/students', linkText: 'Manage Students' },
    ];
    const navLinks = [
        { path: '/telecaller', label: 'Home' },
        { path: '/telecaller/students', label: 'Students' },
    ];
    return (
